...
public class DownloadCallback implements Callback {
    ...
    public DownloadCallback(DownloadHandler handler, String filePath, Long completeBytes) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                if (mDownloadHandler != null) {
                    mDownloadHandler.onFailure(e.toString());
                }
            }
        });
    }
            ...
            if (response.isSuccessful()) {
                ...
                try {
                    ...
                    if (call.isCanceled()) {     //判断是主动取消还是别动出错
                        ...
                        LogUtils.log("onResponse saveFile fail=" + e.getMessage()); //打印错误日志~
                        mHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                if (mDownloadHandler != null) {
                                    mDownloadHandler.onFailure("onResponse saveFile fail=" + e.toString());
                                }
                            }
                        });
                    }
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (mDownloadHandler != null) {
                            mDownloadHandler.onFailure("fail status=" + response.code());
                        }
                    }
                });
            }
        ...
        try {
            ...
            while ((len = is.read(buf)) != -1) {
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (mDownloadHandler != null) {
                            mDownloadHandler.onProgress(final_complete_len, total_len);
                        }
                    }
                });
            }
