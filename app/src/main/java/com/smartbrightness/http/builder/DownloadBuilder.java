...
import com.smartbrightness.http.progress.DownloadProgress;
import com.smartbrightness.http.callback.DownloadCallback;
import com.smartbrightness.http.handle.DownloadHandler;
...
public class DownloadBuilder {
    ...
    public DownloadBuilder(OkHttpUtils okHttpUtils) {
     ...
     * @param handler 下载回调
     */
    public Call enqueue(final DownloadHandler handler) {
        try {
            if (mUrl.length() == 0) {
            ...
            Call call = mOkHttpUtils.getOkHttpClient().newBuilder()
                    .addNetworkInterceptor(new Interceptor() {      //设置拦截器
                        @Override
                        public Response intercept(Chain chain) throws IOException {
                            Response originalResponse = chain.proceed(chain.request());
                            return originalResponse.newBuilder()
                                    .body(new DownloadProgress(originalResponse.body(), handler))
                                    .build();
                        }
                    })
                    .build()
                    .newCall(request);
            call.enqueue(new DownloadCallback(handler, mFilePath, mCompleteBytes));

            return call;
        } catch (Exception e) {
            LogUtils.log("Download enqueue error:" + e.getMessage());
